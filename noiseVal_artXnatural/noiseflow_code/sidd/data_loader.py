...
def gen_patches_png(file_name):
    ...
    for i in range(0, h - patch_size + 1, stride):
        for j in range(0, w - patch_size + 1, stride):
            x = img[i:i + patch_size, j:j + patch_size, :]  # first dim will be removed
            # data aug
            for k in range(0, aug_times):
                # x_aug = data_aug(x, mode=np.random.randint(0, 8))
                if patches is None:
...
def load_data(data_dir='/shared-data/SIDD_Medium_Raw/Data/', verbose=False):
    ...
    file_list = glob.glob(os.path.join(data_dir, '**/*.png'),recursive=True) 
    # initialize
    data1 = None
    cam_iso_info = []
    # generate patches
