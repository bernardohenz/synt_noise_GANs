...
from .gaussian_blur import SimpleGaussian, weights_init_Gaussian
...
class CycleGANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.netG_A = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.norm,
                                        not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids, noise_generator=True)
        self.netG_B = networks.define_G(opt.output_nc, opt.input_nc, opt.ngf, opt.netG, opt.norm,
                                        not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids, noise_generator=False)
        ...
        if len(self.gpu_ids) > 0:
            ...
            self.netGaussian = torch.nn.DataParallel(self.netGaussian, self.gpu_ids)  # multi-GPUs
    ...
    def backward_G(self):
        ...
        if (self.opt.compare_noisy_versions == 'std'):
            self.loss_cycle_A = self.criterionCycle( std_for_channel(self.rec_A),std_for_channel(self.real_A)) * lambda_std
            self.loss_cycle_B = self.criterionCycle( std_for_channel(self.rec_B),std_for_channel(self.real_B)) * lambda_std
        elif (self.opt.compare_noisy_versions == 'std_high'):
            blur_rec_A = self.netGaussian.forward(self.rec_A)
            blur_rec_B = self.netGaussian.forward(self.rec_B)
            self.loss_cycle_A = self.criterionCycle( std_for_channel(self.rec_A-blur_rec_A),std_for_channel(self.real_A-self.blur_real_A)) * lambda_std
            self.loss_cycle_B = self.criterionCycle( std_for_channel(self.rec_B-blur_rec_B),std_for_channel(self.real_B-self.blur_real_B)) * lambda_std
        else:
        ...
        # combined loss and calculate gradients
        self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle_A + self.loss_cycle_B + self.loss_idt_A + self.loss_idt_B + self.loss_low_freq_A + self.loss_low_freq_B
